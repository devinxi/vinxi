...
describe("AnimatePresence", () => {
  ...
  test("Suppresses initial animation if `initial={false}`", async () => {
    ...
    expect(element).toHaveStyle("transform: translateX(100px) translateZ(0)");
  });
          ...
          <AnimatePresence>
            {isVisible && (
              <motion.div
                ...
                onAnimationComplete={() => resolve(parentOpacityOutput.length)}
              >
  ...
  test("Exit variants are triggered with `AnimatePresence.custom`, not that of the element.", async () => {
    ...
    const Component = ({
      ...
      return (
        <AnimatePresence custom={2} onExitComplete={onAnimationComplete}>
          {isVisible && (
            <motion.div
              custom={1}
              variants={variants}
...
describe("AnimatePresence with custom components", () => {
  ...
  test("Animates out a component when its removed", async () => {
    ...
    const Component = ({ isVisible }: { isVisible: boolean }) => {
      return (
        <AnimatePresence>{isVisible && <CustomComponent />}</AnimatePresence>
      );
    };
