...
describe("useQuery", () => {
  ...
  it("should return the correct states for an unsuccessful query", async () => {
    ...
    function Page() {
      const state = useQuery<string[], string, undefined>(key, () => Promise.reject("rejected"), {
        retry: 1,
        retryDelay: 1
      });
  ...
  it("should not pass stringified variables to query function", async () => {
    ...
    const states: CreateQueryResult<QueryKey>[] = [];
  ...
  it("should restart when observers unmount and remount while waiting for a retry when query was cancelled in between (#3031)", async () => {
    ...
    function App() {
      ...
      return (
        <div>
          <button onClick={toggle}>{show ? "hide" : "show"}</button>
          <button onClick={() => queryClient.cancelQueries({ queryKey: key })}>cancel</button>
          {show && <Page />}
        </div>
      );
    }
  ...
  it("should keep initial data when the query key changes", async () => {
    const key = queryKey();
    const states: CreateQueryResult<{ count: number }>[] = [];
  ...
  it("should not retry if retry function `false`", async () => {
    ...
    function Page() {
      const { status, failureCount, error } = useQuery<undefined, string, [string]>(key, queryFn, {
        retryDelay: 1,
  ...
  it("should support dependent queries via the enable config option", async () => {
    ...
    await waitFor(() => rendered.getByText("Status: loading"));
    await waitFor(() => [rendered.getByText("Status: success"), rendered.getByText("Data: data")]);
  });
  ...
  it("should initialize state properly, when initialData is falsy", async () => {
    const key = queryKey();
    const results: CreateQueryResult<number>[] = [];
  ...
  it("should cancel the query if the signal was consumed and there are no more subscriptions", async () => {
    const key = queryKey();
    const states: CreateQueryResult<string, unknown>[] = [];
