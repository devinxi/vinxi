...
import { useQuery, QueryClient, QueryClientProvider, QueryCache, dehydrate, hydrate } from "../..";
...
describe("Server side rendering with de/rehydration", () => {
  it("should not mismatch on success", async () => {
    ...
    function SuccessComponent() {
      ...
      return <PrintStateComponent componentName="SuccessComponent" result={result} />;
    }
    ...
    await prefetchClient.prefetchQuery("success", () => fetchDataSuccess("success"));
    ...
    const expectedMarkup = "SuccessComponent - status:success fetching:true data:success";
  ...
  it("should not mismatch on error", async () => {
    ...
    function ErrorComponent() {
      ...
      return <PrintStateComponent componentName="ErrorComponent" result={result} />;
    }
    ...
    const expectedMarkup = "ErrorComponent - status:loading fetching:true data:undefined";
    ...
    expect(el.innerHTML).toBe("ErrorComponent - status:error fetching:false data:undefined");
  ...
  it("should not mismatch on queries that were not prefetched", async () => {
    ...
    const expectedMarkup = "SuccessComponent - status:loading fetching:true data:undefined";
    ...
    await sleep(50);
    expect(fetchDataSuccess).toHaveBeenCalledTimes(1);
    expect(el.innerHTML).toBe("SuccessComponent - status:success fetching:false data:success!");
