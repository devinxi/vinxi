...
import { queryKey, sleep, mockConsoleError, renderWithClient, setActTimeout, Blink } from "./utils";
import {
  useInfiniteQuery,
...
describe("useInfiniteQuery", () => {
  ...
  it("should return the correct states for a successful query", async () => {
    ...
    function Page() {
      const state = useInfiniteQuery(key, ({ pageParam = 0 }) => Number(pageParam), {
  ...
  it("should be able to refetch when providing page params manually", async () => {
    ...
    function Page() {
      const state = useInfiniteQuery(key, ({ pageParam = 10 }) => Number(pageParam));
  ...
  it("should silently cancel an ongoing fetchNextPage request when another fetchNextPage is invoked", async () => {
    ...
    const fetchPage = jest.fn<Promise<number>, [QueryFunctionContext<string, number>]>(
  ...
  it("should set hasNextPage to false if getNextPageParam returns undefined", async () => {
    ...
    function Page() {
      const state = useInfiniteQuery(key, ({ pageParam = 1 }) => Number(pageParam), {
  ...
  it("should render correct states even in case of concurrent renders with different properties", async () => {
    ...
    const originalUseEffect = createEffect;
    ...
    function Page() {
      ...
      if (concurrent) {
        createEffect = originalUseEffect;
      }
  ...
  it("should build fresh cursors on refetch", async () => {
    ...
    const genItems = (size: number) => [...new Array(size)].fill(null).map((_, d) => d);
    ...
    function Page() {
      ...
      const { status, data, error, isFetchingNextPage, fetchNextPage, hasNextPage, refetch } =
        useInfiniteQuery<Result, Error>(
          key,
          ({ pageParam = 0 }) => fetchItemsWithLimit(pageParam, fetchCountRef.current++),
          {
  ...
  it("should compute hasNextPage correctly for falsy getFetchMore return value on refetching", async () => {
    ...
    function Page() {
      const fetchCountRef = useRef(0);
      const [isRemovedLastPage, setIsRemovedLastPage] = React.useState<boolean>(false);
      const {
        status,
        data,
        error,
          ...
          {status === "loading" ? (
            ...
            <>
              ...
              <div>
                ...
                <button onClick={() => refetch()}>Refetch</button>
                <button onClick={() => setIsRemovedLastPage(true)}>Remove Last Page</button>
              </div>
