 ...
 */
export type MergedInputWithSettings<
  V,
  Settings = {},
  K extends string = "value"
> = V | InputWithSettings<V, Settings, K>;
...
export type OnChangeHandler = (
  value: any,
  path: string,
  context: OnChangeHandlerContext
) => void;
...
export type SchemaToValues<
  Schema,
  IncludeTransient extends boolean = false
> = BeautifyUnionType<UnionToIntersection<Tree<IncludeTransient, Schema>>>;
...
type Tree<
  IncludeTransient extends boolean,
  Leaf,
  LeafKey extends string | number | symbol = ""
> = {
  ...
  2: {
    [Key in LeafKey]: Leaf extends { optional: true }
      ? PrimitiveToValue<Leaf> | undefined
      : PrimitiveToValue<Leaf>;
  };
  // recursivity
  3: {
    [Key in keyof Leaf]: Join<
      Leaf,
      Key,
      Tree<IncludeTransient, Leaf[Key], Key>
    >;
  }[keyof Leaf];
  // dead end
  4: EndLeaf;
  ...
  ? Leaf extends object
    ? 3
    : 4 // if an input has the onChange property then it's transient and isn't returned
...
export interface Plugin<Input, Value = Input, InternalSettings = {}> {
  ...
  normalize?: (
    ...
    path: string,
    data: Data
  ) => { value: Value; settings?: InternalSettings };
  /**
