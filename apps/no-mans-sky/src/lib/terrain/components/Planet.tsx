import { NoiseGenerator, NoiseParams, noiseParamsFolder, useNoiseGenerator } from "@/lib/noise";
import { useControls, folder } from "@/lib/leva";

import { FixedHeightGenerator, NoisyHeightGenerator } from "../lib/height-generator";
import { COLORS, HyposymetricTintsGenerator } from "../lib/color-generator";
import { spline } from "@/lib/leva-spline/Spline";
import * as THREE from "three";
import { createMemo, For, MeshStandardMaterialProps, Overwrite, useFrame } from "solid-three";
import {
  DIRECTIONS,
  PlanetConfig,
...
export function PlanetMaterial(props: MeshStandardMaterialProps) {
  ...
  return <primitive attach="material" object={planetMaterial} {...props} {...materialControls} />;
}

export const PlanetMesh = forwardRef<PlanetMeshImpl, PlanetMeshProps & { object?: PlanetMeshImpl }>(
  function (props, ref) {
...
export function createHeightGenerator(name: string, params: NoiseParams) {
...
function PlanetFace({
  ...
  useFrame(() => {
    ...
    if (Object.keys(difference).length === 0 && Object.keys(toDelete).length === 0) {
      return;
    }
...
export function PlanetSphere({ position, ...props }: PlanetMeshProps & { planet: PlanetConfig }) {
  return (
    <>
      <For each={Object.keys(DIRECTIONS)}>
        {key => <PlanetFace {...props} direction={key as keyof typeof DIRECTIONS} />}
      </For>
    </>
  );
}

function createPlanetGenerator(props: PlanetProps) {
  const planetControls = useControls(props.name, {
  ...
  const planetConfig = createMemo(() => ({
    radius: planetControls.radius,
    position: planetControls.position,
    detailLevelDistances: props.planet?.detailLevelDistances ?? [2500, 1000, 400, 150, 70, 30, 10]
  }));

  const colorGenerator = createColorGenerator(props.name, {
  ...
  const heightGenerator = createHeightGenerator(
    props.name,
    Object.assign(
      {
        octaves: 10,
  ...
  return {
    ...
    heightGenerator,
    get worker() {
      return controls.worker;
    },
    settings: controls,
    get resolution() {
      return planetControls.resolution;
    },
    planet: planetConfig
  };
}
...
export function Planet(props: PlanetProps) {
  return <PlanetSphere {...props} {...createPlanetGenerator(props)} />;
}
...
export function PlanetPlane(props: PlanetProps & { width: number }) {
  const { colorGenerator, heightGenerator, worker, settings, resolution, planet } =
        ...
        [...new Array(chunksZ).fill(0)].map((_, z) => {
          return (
            <PlanetMesh
              ...
              frustumCulled={false}
              key={`${x}.${z}`}
              offset={[chunkSize * x - props.width / 4, 0, chunkSize * z - props.width / 4]}
            />
          );
        })
      )}
