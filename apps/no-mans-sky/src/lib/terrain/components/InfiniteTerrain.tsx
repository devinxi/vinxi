...
import { TerrainMesh as TerrainMeshImpl, TerrainMeshProps } from "../lib/TerrainMesh";
import { useViewer } from "../../../engine/react/Demo";

import { TerrainMaterial } from "./TerrainMaterial";
import { QuadTree2 } from "@/lib/quadtree";
import { dicts } from "@/lib/dicts";
import { createColorGenerator, createHeightGenerator } from "./Planet";
...
export function InfiniteTerrain({ maxViewDistance = 400, chunkSize = 200, resolution = 64 }) {
  const { heightGenerator, colorGenerator } = useTerrainGenerators();

  const [chunks, setChunks] = React.useState(
    () => ({} as Record<string, { offset: [number, number]; size: number; resolution: number }>)
  );
  ...
  useFrame(() => {
    ...
    setChunks(chunks => {
      ...
      for (var xOffset = -visibleChunks; xOffset <= visibleChunks; xOffset++) {
        for (var zOffset = -visibleChunks; zOffset <= visibleChunks; zOffset++) {
          chunks[`${chunkX + xOffset}.${chunkZ + zOffset}`] = {
            offset: [(chunkX + xOffset) * chunkSize, (chunkZ + zOffset) * chunkSize],
            size: chunkSize,
...
export function useTerrainGenerators() {
  const heightGenerator = createHeightGenerator("terrain", {
    octaves: 10,
    persistence: 0.5,
    lacunarity: 1.6,
    exponentiation: 7.5,
  ...
  const colorGenerator = createColorGenerator("terrain", {
    maxElevation: 900.0,
    // planet: {}
    gradient: [
      [COLORS.DEEP_OCEAN, 0],
...
export function QuadTreeTerrain({
  ...
  const [chunks, setChunks] = React.useState(() => ({} as Record<string, TerrainChunkParams>));
  const [toDelete, setToDelete] = React.useState(() => ({} as Record<string, TerrainChunkParams>));
