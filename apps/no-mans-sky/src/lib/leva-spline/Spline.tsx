...
import { GradientInternalPoint, GradientPoint, GradientProps } from "./spline-types";
...
export function ColorComponent() {
  const { value, label, onChange, onUpdate, settings } = useInputContext<GradientProps>();
  ...
  function setColorStopWeight(index: any, v: any) {
    onUpdate([
      ...value.map((a, i) => (i === index ? [a[0], Math.min(Math.max(v, 0), 1.0)] : [...a]))
    ]);
  }
        ...
        <Row dir="column" className="space-y-2">
          ...
          {toggled
            ? value.map(([color, weight], index) => (
                <div className="flex flex-row w-full items-center space-x-2" key={index}>
...
export const StyledButton = css("button", {
  display: "flex",
  alignItems: "center",
