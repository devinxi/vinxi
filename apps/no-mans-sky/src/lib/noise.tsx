...
export class NoiseGenerator {
  ...
  get(x: number, y: number, z?: number): number {
    if (z === undefined) {
      return get2DNoise({ ...this.params, noiseFunction: this.noise[this.noiseType] }, x, y);
    } else {
      return get3DNoise({ ...this.params, noiseFunction: this.noise[this.noiseType] }, x, y, z);
    }
  }
}

function get2DNoise(params: NoiseParams & { noiseFunction: NoiseFunction }, x: number, y: number) {
  ...
  for (let o = 0; o < params.octaves; o++) {
    const noiseValue = params.noiseFunction.noise2D(xs * frequency, ys * frequency) * 0.5 + 0.5;
...
function get3DNoise(
  ...
  for (let o = 0; o < params.octaves; o++) {
    const noiseValue =
      params.noiseFunction.noise3D(xs * frequency, ys * frequency, zs * frequency) * 0.5 + 0.5;
...
export function useNoiseGenerator(name: string, params: Partial<NoiseParams> = {}) {
