 ...
 */
export async function renderErrorsAsHTML(
  ctx: Context<{}, { assetPath: string }>,
  ...
  try {
    ...
    if (!ctx.response.body || (ctx.response.body as { message: string }).message === undefined) {
      console.error(
        "Could not render error as HTML, is the middleware registered before the JSON error middleware?"
      );
      throw e;
    }
