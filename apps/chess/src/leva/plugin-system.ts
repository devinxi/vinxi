...
export function register<Input, Value, InternalSettings, Settings>(
  type: LevaInputs,
  {
    schema,
    ...plugin
  }: InternalPlugin<Input, Value, Settings, InternalSettings>
) {
  if (type in Plugins) {
...
const getUniqueType = () =>
  "__CUSTOM__PLUGIN__" + Math.random().toString(36).substr(2, 9);
 ...
 * @param plugin
 */
export function createPlugin<Input, Value, InternalSettings>(
  plugin: Plugin<Input, Value, InternalSettings>
) {
...
export function format<Settings extends object>(
  type: string,
  value: any,
  settings?: Settings
) {
