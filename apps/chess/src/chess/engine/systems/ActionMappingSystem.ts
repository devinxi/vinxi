...
import { defineSystem, World } from "../ecs";
import { InputValue } from "../react/useInput";
  ...
  [ActionType.Vector2]: {
    ...
    bindings: {
      [BindingType.Axes]: (
        ...
        value.set(input.get(x)?.value[0] || 0, input.get(y)?.va || 0);
      },
...
export default async function ActionMappingSystem(
  world: World
) {
